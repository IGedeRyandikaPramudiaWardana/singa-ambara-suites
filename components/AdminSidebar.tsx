...
export default function AdminSidebar({ activeTab, setActiveTab }: AdminSidebarProps) {
  ...
  useEffect(() => {
    ...
    const storedRole = localStorage.getItem("user_role"); // Nanti kita simpan ini pas login
    if (storedRole) setRole(storedRole);
  }, []);

  const menuItems = [
    { id: "dashboard", label: "📊 Dashboard", desc: "Ringkasan & Statistik", show: true },
    // HANYA TAMPILKAN JIKA SUPER ADMIN
    { id: "users", label: "👥 Kelola Pengguna", desc: "Super Admin Only", show: role === 'super_admin' },
    { id: "checkin", label: "🛎️ Kelola Check-In", desc: "Tamu akan datang", show: true },
    { id: "checkout", label: "👋 Kelola Check-Out", desc: "Tamu sedang menginap", show: true },
    { id: "payments", label: "💰 Detail Pembayaran", desc: "Riwayat Transaksi", show: true },
    { id: "rooms", label: "🛏️ Kelola Kamar", desc: "List & Status Kamar", show: true },
  ];
  ...
  return (
    <div className="w-64 bg-[#1A2225] h-screen fixed left-0 top-0 text-white flex flex-col shadow-2xl z-50">
      <div className="p-6 border-b border-gray-800 flex items-center gap-3">
        ...
        <div><h1 className="font-serif text-[#D4AF37] text-lg tracking-wide">Singa Ambara</h1><p className="text-[10px] text-gray-400 uppercase tracking-widest">{role === 'super_admin' ? 'Super Admin' : 'Admin Staff'}</p></div>
      </div>
